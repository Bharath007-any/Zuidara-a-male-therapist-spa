...
const INIT_BOOKINGS = [
  { id: 1, customer: "Sneha K.", service: "Swedish Relaxation", therapist: "Bharath Kumar", date: "2026-06-16", slot: "10:00 AM", amount: 1499, status: "completed", phone: "+91 99887 66554" },
  { id: 2, customer: "Divya M.", service: "Hot Stone", therapist: "Rajan Pillai", date: "2026-06-16", slot: "2:30 PM", amount: 2299, status: "confirmed", phone: "+91 98765 11223" },
  { id: 3, customer: "Roshini T.", service: "Aromatherapy", therapist: "Bharath Kumar", date: "2026-06-17", slot: "11:30 AM", amount: 1799, status: "pending", phone: "+91 91122 33445" },
  { id: 4, customer: "Meera S.", service: "Deep Tissue", therapist: "Bharath Kumar", date: "2026-06-17", slot: "4:00 PM", amount: 1999, status: "confirmed", phone: "+91 93344 55667" },
  { id: 5, customer: "Preethi V.", service: "Balinese Ritual", therapist: "Rajan Pillai", date: "2026-06-18", slot: "1:00 PM", amount: 2799, status: "pending", phone: "+91 87654 32109" },
];
...
const BOT_FLOWS = {
  ...
  confirm: { msg: "🎉 Your session is confirmed! A therapist will call you 30 minutes before arrival. See you soon! 💆‍♀️", options: ["Book another", "Back to home"] },
};
...
const Card = ({ children, style = {} }) => (
  <div style={{ background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 6, padding: 16, marginBottom: 12, ...style }}>{children}</div>
);
...
export default function ZuidaraSpa() {
  // Global state
  const [therapists, setTherapists] = useState(INIT_THERAPISTS);
  ...
  const [botFlow, setBotFlow] = useState("welcome");
  ...
  const handleBotSend = () => {
    ...
    const msg = botInput.trim(); setUserBotInput("");
  ...
  const setUserBotInput = (v) => setBotInput(v);

  // Derived stats
  const totalRevenue = bookings.filter(b => b.status === "completed").reduce((s, b) => s + b.amount, 0);
  const totalBookings = bookings.length;
  const pendingCount = bookings.filter(b => b.status === "pending").length;
  const todayBookings = bookings.filter(b => b.date === "2026-06-16").length;
    ...
    <div style={{ padding: "20px" }}>
      ...
      {bookStep === 2 && (
        <>
          ...
          <GoldBtn onClick={() => { if (customerName && customerPhone) setBookStep(3); }} style={{ width: "100%", marginTop: 8 }}>Continue →</GoldBtn>
      ...
      {bookStep === 4 && !bookingDone && (
        <>
          <div style={{ fontSize: 18, color: C.text, marginBottom: 20, fontStyle: "italic" }}>When would you like to arrive?</div>
          ...
          {selectedSlot && (
            <Card style={{ marginTop: 20, borderColor: C.borderGold }}>
              ...
              {[["Customer", customerName], ["Service", selectedServiceObj?.name], ["Therapist", selectedTherapistObj?.name], ["Time", `Today, ${selectedSlot}`]].map(([l, v]) => (
              ...
              <GoldBtn onClick={() => {
                const newBooking = { id: bookings.length + 1, customer: customerName, service: selectedServiceObj?.name, therapist: selectedTherapistObj?.name, date: "2026-06-16", slot: selectedSlot, amount: selectedServiceObj?.price, status: "confirmed", phone: customerPhone };
                setBookings(p => [...p, newBooking]);
                setBookingDone(true);
              }} style={{ width: "100%", marginTop: 16 }}>Confirm Booking →</GoldBtn>
            </Card>
          )}
      ...
      {bookingDone && (
        <div style={{ textAlign: "center", paddingTop: 40 }}>
          ...
          <div style={{ fontSize: 13, color: C.textMuted, lineHeight: 1.8, marginBottom: 24 }}>
            ...
            at <strong style={{ color: C.text }}>{selectedSlot}</strong> is confirmed.<br />
    ...
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
        {[
          ...
          ["Today's Sessions", todayBookings, C.green, "🗓"],
        ...
        <Card key={b.id} style={{ padding: "12px 14px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: C.textFaint, marginTop: 2 }}>{b.therapist}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 14, color: C.gold, fontWeight: 700, marginBottom: 4 }}>₹{b.amount.toLocaleString()}</div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["all", "pending", "confirmed", "completed"].map(s => (
            <button key={s} style={{ background: C.bg2, border: `1px solid ${C.border}`, color: C.textMuted, padding: "4px 10px", borderRadius: 20, fontSize: 9, cursor: "pointer", fontFamily: "inherit", letterSpacing: 1, textTransform: "uppercase" }}>{s}</button>
          ))}
        </div>
      </div>
      {bookings.map(b => (
        <Card key={b.id} style={{ padding: "14px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 16, color: C.gold, fontWeight: 700 }}>₹{b.amount.toLocaleString()}</span>
            <div style={{ display: "flex", gap: 8 }}>
  ...
  const renderDev = () => (
    <div style={{ padding: "20px" }}>
      ...
      <Card>
        {[["App", "Zuidara Spa"], ["Version", "2.0.0"], ["Built with", "React + Claude AI"], ["Deployed", "Pending"], ["Platform", "Web / PWA"]].map(([l, v]) => (
      ...
      {[
        ...
        [false, "WhatsApp API integration"],
        [false, "Razorpay payment gateway"],
        [false, "Google Calendar sync"],
        [false, "SMS confirmation"],
        [false, "Instagram DM bot"],
      ...
      <SectionTitle>Instagram Growth Tips</SectionTitle>
      {[
        ["📸", "Post before/after atmosphere reels 3x/week"],
        ["💬", "Use DM automation to reply to every comment"],
        ["#️⃣", "Use: #BengaluruSpa #HomeMassage #SpaAtHome #WellnessBengaluru"],
        ["🎁", "Run a 'Tag a friend' giveaway every month"],
        ["📍", "Tag Bengaluru neighbourhood locations in every post"],
        ["⭐", "Ask every client to leave a Google review"],
      ].map(([icon, tip]) => (
        <div key={tip} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: `1px solid ${C.border}22`, fontSize: 12, color: C.textMuted, lineHeight: 1.5 }}>
          <span style={{ fontSize: 16, flexShrink: 0 }}>{icon}</span>{tip}
        </div>
      ))}

      <SectionTitle>Revenue Targets</SectionTitle>
      <Card>
        {[["Month 1 (soft launch)", "₹15,000 – ₹25,000"], ["Month 3 (word of mouth)", "₹45,000 – ₹60,000"], ["Month 6 (scaled)", "₹1,00,000+"]].map(([period, target]) => (
          <div key={period} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>
            <span style={{ color: C.textMuted }}>{period}</span><span style={{ color: C.gold, fontWeight: 700 }}>{target}</span>
          </div>
        ))}
      </Card>

      <GoldBtn onClick={() => showToast("Export coming soon — connect your backend first!")} style={{ width: "100%", marginTop: 8 }}>
        Export Data as CSV
      </GoldBtn>
    </div>
  );
